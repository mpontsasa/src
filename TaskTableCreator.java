            ...
            for (int su = 0; su < subUnitTs[unit].length; su++) {
                ...
                if (su == 0 || su == 3){    //material or transport
                    ...
                    for (int row = 1; row < subUnitTs[unit][su].length; row++) {
                        subUnitTs[unit][su][row] = taskModel.getTaskUnits().get(unit).getSubUnits().get(su).getTaskRows().get(row).getTableHeader();  //row -1 mert a modellben 0-tol megy, itt a header miatt 1-tol
                    ...
                    for (int row = 1; row < subUnitTs[unit][su].length; row++) {
                        subUnitTs[unit][su][row] = taskModel.getTaskUnits().get(unit).getSubUnits().get(su).getTaskRows().get(row).getExtendedTableHeader();//row -1 mert a modellben 0-tol megy, itt a header miatt 1-tol
                        subUnitTs[unit][su][row][0] = "" + row; //set index
                    }
                }
            }
